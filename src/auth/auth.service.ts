...
import {
  ...
  verifyFacebookToken,
  verifyGoogleToken,
} from 'src/config/socialmedia.config';
...
export class AuthService {
  ...
  private async generateTokensAndSession(
    ...
    const accessExp = (this.configService.get('ACCESS_TOKEN_EXPIRES_IN') ??
      '15m') as StringValue;

    const refreshExp = (this.configService.get('REFRESH_TOKEN_EXPIRES_IN') ??
      '7d') as StringValue;
  ...
  async signup(dto: SignupDto, req: Request) {
    const { role, mobile, email, password, confirm_password, country_code } =
      dto;
  ...
  async signup_google(
    ...
    if (user && user.role !== role) {
      throw new BadRequestException(`Email already registered as ${user.role}`);
    }
  ...
  async login_google(
    ...
    // Role validation
    if (user.role !== role) {
      throw new UnauthorizedException(`Account registered as ${user.role}`);
    }
    ...
    const authData = await this.generateTokensAndSession(user, user, req);
  ...
  async signup_facebook(
    ...
    const tokens = await this.generateTokensAndSession(user, user, req);
  ...
  async signup_apple(
    ...
    if (!email) {
      throw new BadRequestException('Apple email not available. Try login.');
    }
